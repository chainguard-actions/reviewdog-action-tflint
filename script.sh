...
if [ -n "${INPUT_TFLINT_RULESETS}" ]; then
  while IFS= read -r -d '' RULESET; do
    ...
    echo '::endgroup::'
  done < <(printf '%s' "${INPUT_TFLINT_RULESETS}" | xargs printf '%s\0')
fi
...
echo '::group:: Running tflint with reviewdog 🐶 ...'
  ...
  if [ -n "${INPUT_FLAGS}" ]; then
    while IFS= read -r -d '' t; do tflint_flags+=("$t"); done \
      < <(printf '%s' "${INPUT_FLAGS}" | xargs printf '%s\0')
  fi

  chdir_args=()
  if [ -n "${CHDIR_COMMAND}" ]; then
    chdir_args+=("${CHDIR_COMMAND}")
  fi

  TFLINT_PLUGIN_DIR=${TFLINT_PLUGIN_DIR} "${TFLINT_PATH}/tflint" -c "${INPUT_TFLINT_CONFIG}" --format=checkstyle "${tflint_flags[@]}" "${chdir_args[@]}" \
